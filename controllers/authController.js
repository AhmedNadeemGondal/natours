...
exports.signup = catchAsync(async (req, res, next) => {
  // This part is changed compared to the lectures for now
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) next(new AppError('The user no longer exist', 401));
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forgot password? Submit PATCH request with new password and confirm password to ${resetURL}. If not forgot passowrd => Ignore Message`;
  try {
    await sendEmail({
      email: user.email,
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Set new password ony if token has not expired and there is a user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // ) {
  //   return next(new AppError('Your email or password is not correct', 404));
  // }
  // // 3) If so, update the password
